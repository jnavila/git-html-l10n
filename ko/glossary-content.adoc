...
[[def_fast_forward]]정방향 진행(fast-forward)::
	정방향 진행(fast-forward)은 특정한 유형의 <<def_merge,병합>>으로, 현재 가지고 있는 <<def_revision,리비전>>과 동일한 조상을 가진 다른 <<def_branch,분기>>의 변경사항을 "병합"하는 경우를 말합니다. 이 경우, 새로운 <<def_merge,병합>> <<def_commit,커밋>> 을 만들지 않고 대신 분기를 병합하는 분기와 동일한 리비전을 가리키도록 업데이트합니다. 이러한 상황은 원격 <<def_repository,저장소>>의 <<def_remote_tracking_branch,원격 추적 분기>>에서 자주 발생할 수 있습니다.
