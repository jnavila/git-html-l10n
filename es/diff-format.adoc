...
Todos los comandos imprimen una línea por cada archivo modificado, excepto `git diff-files` en caso de un archivo sin fusionar, el cual imprime dos líneas: una "sin fusionar" y otra "edición in situ".
...
. dos puntos.
. modo para "src"; 000000 si es creación o si "src" es desde el índice y está sin fusionar.
. un espacio.
. modo para "dst"; 000000 si se borra, o si "dst" es desde el índice y está sin fusionar.
. un espacio.
. sha1 para "src"; 0\{40\} si es creación, o si "src" es desde el índice y está sin fusionar.
. un espacio.
. sha1 para "dst"; 0\{40\} si es borrado, o si "dst" es desde el índice y está sin fusionar, o si "dst" es desde el árbol de trabajo y está desincronizado con el índice.
. un espacio.
