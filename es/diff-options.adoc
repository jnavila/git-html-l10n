...
`remerge`::
`r`:: Refusiona confirmaciones de fusión de dos padres para crear un objeto árbol temporal --potencialmente conteniendo ficheros con marcadores de conflicto y similares. Entonces se muestra un diff entre ese árbol temporal y la confirmación de fusión actual.
--
+
...
`--no-indent-heuristic`::
	Desactiva la heurística de sangrado.
...
`--stat[=<anchura>[,<anchura-nombre>[,<recuento>]]]`::
	Genera estadísticas de diferencia. Predeterminadamente, tanto espacio como sea necesario para la parte del nombre del fichero, y el resto para la parte del grafo. El ancho máximo se predetermina al ancho de la terminal, o a 80 columnas si no esta conectado a una, y puede ser anulada con _<ancho>_. El ancho del nombre del archivo puede limitarse dando otro ancho _<ancho-de-nombre>_ después de una coma, o asignando `diff.statNameWidth=<ancho-de-nombre>`. El ancho del grafo puede limitarse con `--stat-graph-width=<ancho-de-grafo>` o asignando `diff.statGraphWidth=<ancho-de-grafo>`. Usar `--stat` o `--stat-graph-width` afecta a todos los comandos que generan una gráfica de estadísticas, mientras que `diff.statNameWidth` y `diff.statGraphWidth` no afectan `git format-patch`. Dando un tercer parámetro _<conteo>_, puedes limitar la salida a las primeras _<conteo>_ líneas, seguida por `...` si hay mas.
+
...
+
La opción `--word-diff` funciona tomando el mismo diff línea a línea producido sin la opción y computa los cambios palabra por palabra en cada bloque. Esto puede producir un diff más largo al que haría una herramienta diff dedicada basada en palabras. Si Git siguiera una implementación diferente en el futuro, la salida podría cambiar. Nótese que esto es similar a la opción `--diff-algorithm`, que también puede modificar la salida.
...
`-D`::
`--irreversible-delete`::
	Omite la preimagen para eliminaciones, p. ej. imprime solo el encabezado pero no la diferencia entre la preimagen y `/dev/null`. El parche resultante no es para aplicarse con `patch` o `git apply`; es únicamente para personas que solo quieren concentrarse en revisar el texto después del cambio. Adicionalmente, es obvio que la salida carece de información suficiente para aplicar tal parche en reversa, incluso manualmente, de ahí el nombre de la opción.
+
...
`--default-prefix`::
ifdef::git-format-patch[]
	Usa los prefijos predeterminados de origen y destino ("a/" y "b/").
	Esto anula variables de configuración como `format.noprefix`,
	`diff.srcPrefix`, `diff.dstPrefix` y `diff.mnemonicPrefix`
	(ver linkgit:git-config[1]).
endif::git-format-patch[]
ifndef::git-format-patch[]
	Usa los prefijos predeterminados de origen y destino ("a/" y "b/").
	Esto anula variables de configuración como `diff.noprefix`,
	`diff.srcPrefix`, `diff.dstPrefix` y `diff.mnemonicPrefix`
	(ver linkgit:git-config[1]).
endif::git-format-patch[]
...
--max-depth=<profundidad>::
	Para cada especificación de ruta dada en la línea de comandos, desciende a lo mucho `<profundidad>` niveles de directorios. Un valor de `-1` significa sin límite. No puede combinarse con comodines en la especificación de ruta. Dado un árbol conteniendo `foo/bar/baz`, la lista siguiente muestra las coincidencias generadas por cada conjunto de opciones:
+
--
 - `--max-depth=0 -- foo`: `foo`
...
--
+
Si no se da una especificación de ruta, la profundidad se mide como si se hubieran especificado todas las entradas del nivel más alto. Nota que eso es distinto a medir desde la raíz, ya que `--max-depth=0` aún devolverá `foo`. Esto te permite seguir limitando la profundidad cuando se pide un subconjunto de las entradas de más alto nivel.
+
Nota que esta opción sólo es soportada para diferencias entre objetos árbol, no contra el índice o el árbol de trabajo.
