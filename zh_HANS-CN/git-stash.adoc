...
git stash export (--print | --to-ref <引用>) [<暂��>...]
git stash import <提交>
...
push [-p|--patch] [-S|--staged] [-k|--[no-]keep-index] [-u|--include-untracked] [-a|--all] [-q|--quiet] [(-m|--message) <信息>] [--pathspec-from-file=<文件> [--pathspec-file-nul]] [--] [<路径规范>...]::
	...
	将你的本地修改保存到一个新的 "贮藏条目 "中，并将它们回滚到 HEAD（在工作区和索引中）。 <信息>部分是可选的，它给出了描述和贮藏的状态。
+
为了快速制作快照，你可以省略 "push"。 在这种模式下，非选项参数是不允许的，以防止拼写错误的子命令产生不需要的贮藏条目。 这方面的两个例外是`stash -p`，它作为`stash push -p`的别名，以及为消除歧义允许在双连字符`--`之后的路径规范元素。

save [-p|--patch] [-S|--staged] [-k|--[no-]keep-index] [-u|--include-untracked] [-a|--all] [-q|--quiet] [<信息>]::

	该选项已被弃用，改为 "git stash push"。 它与 "stash push "的不同之处在于，它不能接受路径规范。 取而代之的是，所有非选项的参数都被串联起来，形成贮藏消息。

list [<日志选项>]::

	列出你目前拥有的贮藏目录。 每个 "贮藏目录 "都列出了它的名字（例如，`stash@{0}`是最新的条目，`stash@{1}`是之前的条目，等等），条目产生时的分支名称，以及该条目所基于的提交的简短描述。
+
----------------------------------------------------------------
...
show [-u|--include-untracked|--only-untracked] [<diff选项>] [<贮藏目录>]::
...
pop [--index] [-q|--quiet] [<暂存>]::
	...
	从贮藏库列表中移除一个单一的贮藏状态，并将其应用于当前工作区状态之上，也就是做`git stash push`的逆向操作。工作区必须与索引匹配。
+
...
branch <分支名> [<贮藏项>]::
	...
	创建并检查一个名为 `<分支名>` 的新分支，从最初创建 `<贮藏项>` 的提交开始，将 `<贮藏项>` 中记录的修改应用到新的工作区和索引。 如果成功了，并且`<贮藏项>`是`stash@{<版本>}`形式的引用，那么它将删除`<贮藏项>`。
+
...
`export ( --print | --to-ref <ref> ) [<stash>...]`::
	...
	将指定的 stash 导出为一个提交链；如果未指定 stash，则导出全部。这个提交链可以通过常规的 fetch 和 push 机制传输，然后再用 `import` 子命令导入。

`import <commit>`::
	从指定提交中导入 stash，该提交必须由 `export` 创建，并将其添加到 stash 列表中。若要替换现有 stash，请先使用 `clear`。
...
`--print`::
	这个选项只对 `export` 命令有效。
+
创建表示已导出 stash 的提交链，但不将其存储到任何 ref 命名空间中，并将对象 ID 输出到标准输出。此选项是为脚本设计的。

`--to-ref`::
	这个选项只对 `export` 命令有效。
+
创建表示已导出 stash 的提交链，并将其存储到指定 ref。
...
+
一个形式为`stash@{<版本>}`的引用。如果没有给出`<贮藏项>`，则假定是最新的贮藏库（即`stash@{0}`）。

讨论
--

一个贮藏库条目被表示为一个提交，它的目录树记录了工作区的状态，它的第一个父节点是创建该条目时在`HEAD`的提交。 第二个父节点的树记录了条目生成时索引的状态，它是`HEAD`提交的一个子节点。 祖先图看起来像这样：
...
如果你错误地丢弃或清除了贮藏库条目，它们无法通过正常的安全机制恢复。 然而，你可以试试下面的咒语，以获得仍在你的仓库中，但无法到达的贮藏库条目列表：
+
----------------------------------------------------------------
