...
+
NOTE: 注意：你可以在仓库配置中指定默认的漂亮格式（见linkgit:git-config[1]）。
...
当命令行未提供 `--pretty`、`--format` 或 `--oneline` 选项时，这是 `git log`、`git show` 和 `git whatchanged` 命令的默认值。
endif::with-breaking-changes[]
ifdef::with-breaking-changes[]
当命令行未提供 `--pretty`、`--format` 或 `--oneline` 选项时，这是 `git log` 和 `git show` 命令的默认值。
endif::with-breaking-changes[]
+
