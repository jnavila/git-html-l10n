...
除非另有说明，大多数选项和操作模式要求您在 git 仓库内部或由 git 仓库控制的个工作区中运行此命令，否则将给出致命错误。
...
--parseopt::
	在选项解析模式下使用 'git rev-parse'（参见下面的 PARSEOPT 部分）。在此模式下，即使不在仓库内部或由仓库控制的工作区中，也可以使用该命令。
...
--prefix <参数>::
	就像从工作区的 `<参数>` 子目录调用 'git rev-parse'一样。 任何相对文件名都会被解析为以 `<参数>` 为前缀，并以这种形式打印。
+
...
+
如果您想确保输出结果确实命名了对象数据库中的对象，并且/或者可以用作您需要的特定类型的对象，您可以在参数中添加 `^{type}` 剥离运算符。 例如，`git rev-parse "$VAR^{commit}"` 将确保 `$VAR` 命名的现有对象是一个类似提交的对象（即一个提交，或一个指向提交的注释标签）。 要确保 `$VAR` 命名的是任何类型的现有对象，可以使用 `git rev-parse "$VAR^{object}"。
+
...
--symbolic-full-name::
	这与 --symbolic 类似，但它会省略非引用输入（即分支或标签名称；或更明确的消歧义 "heads/master" 形式，当你想命名 "master" 分支时，却有一个不幸命名为 "master" 的标签），而显示为完整的引用名称（例如 "refs/heads/master"）。
...
--tags[=<模式>]::
--remotes[=<模式>]::
	分别显示所有分支、标签或远程跟踪分支（即分别在 `refs/heads`、`refs/tags` 或 `refs/remotes`中找到的引用）。
+
...
--path-format=(absolute|relative)::
	控制某些其他选项的行为。如果指定为绝对路径，这些选项打印的路径将是绝对和规范的。如果指定为相对路径，路径将相对于当前工作区（如果可能）。 默认情况下，路径将与选项相关。
+
...
--git-dir::
	如果已定义，则显示 `$GIT_DIR`。否则显示 .git 目录的路径。如果是相对路径，则显示当前工作区的相对路径。
+
...
--resolve-git-dir <路径>::
	检查 <路径> 是否为有效的仓库或指向有效仓库的 gitfile，并打印仓库的位置。 如果 <路径> 是 gitfile，则打印指向真实仓库的解析路径。
...
--show-toplevel::
	显示工作区顶层目录的路径（默认为绝对路径）。如果没有工作区，则会报错。
...
--is-inside-git-dir::
	当当前工作区低于仓库目录时，打印 "true"，否则打印 "false"。

--is-inside-work-tree::
	当当前工作区位于仓库工作区内时，打印 "true"，否则打印 "false"。
