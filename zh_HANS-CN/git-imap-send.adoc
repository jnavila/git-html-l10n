...
-f <文件>::
--filter=<过滤器定义>::
	指定用于保存邮件的文件夹。例如：`--folder=[Gmail]/Drafts` 或 `-f INBOX/Drafts`。
...
如果你使用的是 OAuth2.0 认证，使用 credential helper 生成 token 会更方便。linkgit:git-send-email[1] 中建议的 credential helper 同样可以用于 `git imap-send`。
