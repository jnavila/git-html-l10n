...
+
WARNING: 历史遍历机制假定提交的提交时间戳是非递减的。你应当先考虑是否真的需要使用该选项。即便如此，也只应在如下情况下用它覆盖 committer 日期：你要应用的提交是建立在某个基础提交之上，而该基础提交（按提交日期计）比你正在应用的最旧补丁还要更早。
...
. 手解决工作区中的冲突，并更新索引文件，使其进入补丁应该产生的状态。 然后用 `--continue` 选项运行该命令。
