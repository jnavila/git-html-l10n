...
`-n<num>`::
	_<num>_ indique combien de lignes de l'annotation, le cas échéant, sont imprimées lorsque l'on utilise `-l`. Implique `--list`.
+
...
`--color[=<quand>]`::
	Respecter toutes les couleurs spécifiées dans l'option `--format`. Le champ _<quand>_ doit être un des valeur `always`, `never`, ou `auto` (si _<quand>_ est absent, se comporter comme si `always` était donné).
