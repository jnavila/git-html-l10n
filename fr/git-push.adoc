...
+
Le _<dst>_ indique quelle référence du côté distant est mise à jour avec cette poussée. Des expressions arbitraires ne peuvent pas être utilisées ici, une référence réelle doit être nommée. Si `git push [<dépôt>]` sans aucun argument `<spéc-de-réf>` est défini pour mettre à jour une référence à la destination avec `<src>` avec la variable de configuration `remote. <dépôt> .push`, la partie `:<dst >` peut être omise - une telle poussée mettra à jour une référence que `<src>` met normalement à jour sans aucun `<spéc-de-réf>` sur la ligne de commande. Sinon, un `:<dst>` manquant signifie mettre à jour la même référence que le `<src>`.
+
