...
`git-diff-index <arbre-esque>`::
	compare l'_<arbre-esque>_ et les fichiers du système de fichiers.

`git-diff-index --cached <arbre-esque>`::
	compare l'_<arbre-esque>_ et l'index.
...
Toutes les commandes affichent une ligne de sortie par fichier modifié, excepté `git diff-files` dans le cas d'un fichier non-fusionné, qui affiche à la fois une ligne "non-fusionné" et "édité en place".
...
. deux points.
. le mode pour "src" ; 000000 si c'est une création ou si "src" vient de l'index ou n'est pas fusionné.
. un espace.
. mode pour "dst" ; 000000 si suppression ou non-fusionné, ou si "dst" vient de l'index ou n'est pas fusionné.
. un espace.
. sha1 de "src", 0\{40\} si création ou non fusionné ou si "src" vient de l'index et n'est pas fusionné.
. un espace.
. sha1 de "dst", 0\{40\} si suppression si "dst" vient de l'index et n'est pas fusionné ou si "dst" vient de l'arbre de travail et est désynchronisé par rapport à l'index.
. un espace.
