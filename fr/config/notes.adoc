...
`notes.displayRef`::
	Depuis quelle réf (ou réfs, si un glob ou si spécifié plus d'une fois), en plus de la configuration réglée par défaut par `core.notesRef ' ou `GIT_NOTES_REF ' , lire les notes lors de l'affichage des messages de validations avec la famille de commandes `git log`.
+
